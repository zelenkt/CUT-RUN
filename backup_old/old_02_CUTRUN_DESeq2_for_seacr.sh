...
# extract raw reads
if [[ ! -f ${target_dir}/DESeq2_merged_all_peaks/final_cat_merge_001_non_stringent.stringent.bed ]]; then
    printf "Generating coverage files with raw read number for ...\n" "$base"

    sampleCount=$(awk -F',' 'NR>1{print $1}' $samples | wc -l); awk -F',' 'NR>1{print $1}' $samples | sort > tmp_4245277sampleNames.txt; for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p tmp_4245277sampleNames.txt);   echo cp ${sampleName}/seacr_peaks/${sampleName}_seacr_001_non_stringent.stringent.bed DESeq2_merged_all_peaks/ ; done
    
    
    # awk '{print $7}' ${sampleName}${suffix} > tmp_3541643543534154_${sampleName}; echo -e '' > tmp_9878798798798_consensus_${sampleOrder}; awk -v OFS="_" '{print $1,$2,$3}' ${sampleName}${suffix} >> tmp_9878798798798_consensus_${sampleOrder}; done

    # cat *_001_non_stringent.stringent.bed | sortBed | mergeBed -c 4,5,6 -o sum,max,collapse | sortBed > final_h3k27me3_cat_merge_001_non_stringent.stringent.bed


    # for seacr_sample in "${macs2_pvalues[@]}"
    #     do
