...
suffix="${f:-_seacr_merged_peaks_coverage.bed}"
...
##### SET VARIABLES #####
target_dir="${d:-${PWD}/DESeq2_merged_all_peaks/DESeq2_seacr}"
if [[ "$target_dir" == "$PWD/DESeq2_merged_all_peaks/DESeq2_seacr" && ! -d "$target_dir" ]]; then # IF TARGET DIR DOES NOT EXIST
    mkdir -p "$target_dir"
...
[[ -s "$samples" ]] || { printf "%s is empty or does not exist. Please specify valid CUT&RUN sample list file and rerun %s. Exiting...\n" "$OPTARG" "$(basename $0)"; exit 1; } # VERIFY SAMPLE LIST FILE IS VALID
...
baseline_check=$(awk -F$',' -v condition=$condition -v baseline=$baseline 'BEGIN {s=0} 
    ...
    END {print s}' "$samples")
[[ $baseline_check > 0 ]] || { printf "There are no CUT&RUN samples of type \"%s\" under condition \"%s\". Please select a valid control group and rerun %s. Exiting...\n" "$baseline" "$condition" "$(basename $0)"; exit 1; } #VERIFY THAT AT LEAST ONE SAMPLE UNDER SELECTED CONDITION MATCHES BASELINE TYPE
...
if [[ ! -f ${target_dir}/raw_read_counts_cutrun_consensus_peaks.txt ]]; then
    ...
    sampleCount=$(awk -F',' 'NR>1{print $1}' $samples | wc -l); awk -F',' 'NR>1{print $1}' $samples | sort > tmp_4245277sampleNames.txt; for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p tmp_4245277sampleNames.txt); cp ${sampleName}/seacr_peaks/${sampleName}_seacr_001_non_stringent.stringent.bed DESeq2_merged_all_peaks/ ; done

    mv *.err DESeq2_merged_all_peaks/
    mv *.out DESeq2_merged_all_peaks/
    cd DESeq2_merged_all_peaks
    cat *_seacr_001_non_stringent.stringent.bed | sortBed | mergeBed -c 4,5,6 -o sum,max,collapse | sortBed > final_seacr_cat_merge_001_non_stringent.stringent.bed


    for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p ../tmp_4245277sampleNames.txt); bedtools coverage -a final_seacr_cat_merge_001_non_stringent.stringent.bed -b ../${sampleName}/${sampleName}.bam | uniq > ${sampleName}_seacr_merged_peaks_coverage.bed; done
...
printf "Condition = %s\n" "$condition"
printf "Baseline = %s\n" "$baseline"
printf "CUT&RUN Sample list file = %s\n" "$samples"
