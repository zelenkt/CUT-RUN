...
##### OPTIONAL ARGUMENTS #####
usage="$(basename $0) [-h] [-p] [-d dir] [-n string] [-p PE] -- Program to analyze fastq files from CUT&RUN.

Where:
    -h  Show this help text
    -m  Active 'pre-macs' mode. Halts program after final BAM file is produced. (default: off)
    -d  Set target directory (directory containing CUT&RUN fastq files or their symbolic links) (default: Current directory)
    -n  Set sample base name (i.e. CR_WT1) (default: Name of directory containing files)
...
Example usage:
    ...
    $(basename $0) -d /directory/containing/fastq/files -n CR_WT1 -m -p SE"
...
if [[ $genome == "hg38" ]]; then
  ...
  [[ -f $HOME/lib/human_hg38_chr_list.txt ]] || awk '{if ($1 !~ /[M,Y,_]/) print $1}' $chr_sizes > $HOME/lib/human_hg38_chr_list.txt
  chr_list=$HOME/lib/human_hg38_chr_list.txt
  else
  ...
  if [[ $genome == "hg19" ]]; then
    ...
    [[ -f $HOME/lib/human_hg19_chr_list.txt ]] || awk '{if ($1 !~ /[M,Y,_]/) print $1}' $chr_sizes > $HOME/lib/human_hg19_chr_list.txt
    chr_list=$HOME/lib/human_hg19_chr_list.txt
  else
...
if ! [[ -f "$base".sam || -f "$base"_nodups.bam || -f "$base".bam || -f "$base"_full.bam ]]; then
    ...
    fi # added parameters to the bowtie mapping to be consistent with the original CUT&RUN protocol and recommendations
    printf "Done.\n"
else
...
ml purge
module load MACS2
module load parallel/20180822-foss-2018b # originally used, sometimes causing problems, see below alternatives
#module load parallel/20210722-GCCcore-11.2.0
#module load parallel/20190922-GCCcore-8.3.0

[[ -d "macs2_peaks" ]] || mkdir macs2_peaks

# Compare different p val thresholds for macs peak calling, this method gives narrowpeaks, more suitable for motif analyses but for CUT&RUN we prefer a dedicated tool SEACR below, or combining peaks from both
...
for pval in "${macs2_pvalues[@]}"
    do
        if ! [[ -f macs2_peaks/"$base"_macs2_"$pval"_treat_pileup.bdg || -f macs2_peaks/"$base"_macs2_"$pval"_treat_pileup.bdg.gz ]]; then
            printf "Calling peaks with macs2...\n"
            macs2 callpeak -t "$base".bam --call-summits -g "$macs_genome" -p "$pval" -f BAMPE --SPMR --outdir macs2_peaks -B -n "$base"_macs2_"$pval"
            # version 1 based on UF script from Eric:  macs2 callpeak -t "$base".bam --call-summits --nomodel -g mm -q 0.05 -f BAM --SPMR --outdir macs2_peaks -B -n "$base"
        ...
        if ! [[ -f macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg || -f macs2_peaks/"$base"_macs2_"$pval"_subtract.bw ]]; then
            printf "Cleaning pileup files via subtract...\n"
            macs2 bdgcmp -t macs2_peaks/"$base"_macs2_"$pval"_treat_pileup.bdg -c macs2_peaks/"$base"_macs2_"$pval"_control_lambda.bdg -m subtract -o macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg
            printf "Done.\n"
            ...
            parallel --pipepart -q --block 5M -a macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg awk '$4>0' > "$temp_file" && mv "$temp_file" macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg
            printf "Done.\n"
        else
        ...
        if [[ ! -f macs2_peaks/"$base"_macs2_"$pval"_subtract.bw ]]; then
            temp_file=$(mktemp --tmpdir="$PWD")
            #SORT SUBTRACT FILE
            printf "Sorting %s_subtract.bdg for conversion to bigWig..." "$base"
            sort --parallel=$(nproc) -k1,1 -k2,2n macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg > "$temp_file" && mv "$temp_file" macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg
            printf "Done.\n"
            
            #CONVERT BEDGRAPH TO BIGWIG
            module load Kent_tools
            printf "Converting %s_subtract.bdg to bigWig via bedGraphToBigWig..." "$base"
            bedGraphToBigWig macs2_peaks/"$base"_macs2_"$pval"_subtract.bdg "$chr_sizes" macs2_peaks/"$base"_macs2_"$pval"_subtract.bw
            printf "Done.\n"
        else
...
# for alternative peak calling dedicated to CUT&RUN datasets, use SEACR - this is our preferential method
...
bash SEACR_1.3.sh "$base".fragments.bedgraph 0.01 non stringent "$base"_seacr_001_non_stringent

# I tested also other filtering conditions to identify peaks, but ultimately we care about differential peaks for the histone marks so I left the least stringent p value (0.01) for this step and we will regulated identification of differentially modified site based on DESeq2 p values
# Note that for Bcl11b CUT&RUN we only have the WT set so there is no differential analysis - in this case the 0.01 had too many peaks and we ended up using 0.005 filtering

bash SEACR_1.3.sh "$base".fragments.bedgraph 0.005 non stringent "$base"_seacr_0005_non_stringent
bash SEACR_1.3.sh "$base".fragments.bedgraph 0.001 non stringent "$base"_seacr_0001_non_stringent
