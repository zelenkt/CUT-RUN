...
#SBATCH --time=8:00:00
#SBATCH --job-name=DESeq2_MACS2
#SBATCH --output=DESEq2_MACS2.%j.out
#SBATCH --error=DESEq2_MACS2.%j.err

##### LAST UPDATED 02/25/2025 Tomas Zelenka #####


##### ARGUMENTS AND USAGE #####
usage="$(basename $0) [-h] [-d dir] [-r file] [-t string] [-f string] [-c string]


Optional Arguments:
    -h  Show this help text
    -d  Set target directory (directory to write analysis files) (default: $PWD/DESeq2)
    -s  Set CUT&RUN sample list file (default: /share/lab_avram/HPC_Cluster/user/tomas/08_cut_run_5-30-2023/test/samples.csv)
    -t  Set test condition (column from CUT&RUN sample list file) (i.e. genotype or cell_subset) (default: genotype)
    -c  Set control group (i.e. WT if test condition is genotype, or Th0 if test condition is cell_subset (default: WT)
    -f  Set a suffix to be appended to generated files
...
while getopts ':h:d:s:t:c:' option; do
    ...
    s)  if [[ -s "$OPTARG" ]]; then
            s="$OPTARG"
            printf "CUT&RUN sample list file manually set to: %s\n" "$OPTARG"
        else
            printf "%s is empty or does not exist. Please specify valid CUT&RUN sample list file and rerun %s. Exiting...\n" "$OPTARG" "$(basename $0)"
        fi
        ;;
    t)  t="$OPTARG"
...
suffix="${f:-_macs2_merged_peaks_coverage.bed}"
species="mouse"
...
[[ -d "DESeq2_merged_all_peaks" ]] || mkdir DESeq2_merged_all_peaks


##### SET VARIABLES #####
target_dir="${d:-${PWD}/DESeq2_merged_all_peaks/DESeq2_macs2}"
if [[ "$target_dir" == "$PWD/DESeq2_merged_all_peaks/DESeq2_macs2" && ! -d "$target_dir" ]]; then # IF TARGET DIR DOES NOT EXIST
    mkdir -p "$target_dir"
...
# create a unified set of peaks and extract raw reads
if [[ ! -f ${target_dir}/raw_read_counts_cutrun_consensus_peaks.txt ]]; then
    printf "Generating coverage files with raw read number for ...\n" "$base"

    sampleCount=$(awk -F',' 'NR>1{print $1}' $samples | wc -l); awk -F',' 'NR>1{print $1}' $samples | sort > tmp_4245277sampleNames.txt; for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p tmp_4245277sampleNames.txt); cp ${sampleName}/macs2_peaks/${sampleName}_macs2_0.001_peaks.narrowPeak DESeq2_merged_all_peaks/ ; done

    mv *.err DESeq2_merged_all_peaks/
    mv *.out DESeq2_merged_all_peaks/
    cd DESeq2_merged_all_peaks
    cat *_macs2_0.001_peaks.narrowPeak| sortBed | mergeBed -c 4,5,6,7,8,9,10 -o collapse,mean,collapse,mean,mean,mean,collapse | sortBed > final_macs2_cat_merge_0001_macs2.bed


    for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p ../tmp_4245277sampleNames.txt); bedtools coverage -a final_macs2_cat_merge_0001_macs2.bed -b ../${sampleName}/${sampleName}.bam | uniq > ${sampleName}_macs2_merged_peaks_coverage.bed; done
    rm ../tmp_4245277sampleNames.txt
fi
...
if [[ ! -f ${target_dir}/raw_read_counts_cutrun_consensus_peaks.txt ]]; then
    ...
    sampleCount=$(awk -F',' 'NR>1{print $1}' $samples | wc -l); awk -F',' 'NR>1{print $1}' $samples | sort > tmp_4245277sampleNames.txt; for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p tmp_4245277sampleNames.txt); awk '{print $11}' ${sampleName}${suffix} > tmp_3541643543534154_${sampleName}; echo -e '' > tmp_9878798798798_consensus_${sampleOrder}; awk -v OFS="_" '{print $1,$2,$3}' ${sampleName}${suffix} >> tmp_9878798798798_consensus_${sampleOrder}; done; 
    awk '
    { 
        for (i=1; i<=NF; i++)  {
            a[NR,i] = $i
...
( cd "$target_dir" && Rscript "$HOME"/bin/cut_and_run_pipeline/cutrun_DESeq2.R -c "$condition" -b "$baseline" -s "$samples" -r "$readcounts" ) # RUN DESEQ2.R WITHIN SUBSHELL TO CHANGE DIRECTORY TO ANALYSIS DIR
...
awk '($8 < -0.2 && $9<0.3) || ($8 < -0.2 && $9<0.05) {print $0}' all_differential_peaks_noFilt_for_annotation.bed > down_logFC02_Pval03__Pval005.bed
awk '($8 > 0.2 && $9<0.3)  || ($8 > 0.2 && $9<0.05) {print $0}' all_differential_peaks_noFilt_for_annotation.bed > up_logFC02_Pval03__Pval005.bed
