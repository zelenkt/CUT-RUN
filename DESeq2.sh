...
Example usage:
    sbatch --chdir=/share/lab_avram/HPC_Cluster/user/kyle.lorentsen/RNA/CD8_RNA_Seq_Run --export=t=Cell_subset,c=Th0,bam_list=\"DSS_KO_7158/hisat2/DSS_KO_7158.bam DSS_KO_7168/hisat2/DSS_KO_7168.bam DSS_WT_7021/hisat2/DSS_WT_7021.bam DSS_WT_7177/hisat2/DSS_WT_7177.bam\" $HOME/bin/$(basename $0)
...
( cd "$target_dir" && Rscript "$HOME"/bin/DESeq2.R -c "$condition" -b "$baseline" -r "$RNA_samples" -g "$gtf" -l "$bam_list" ) # RUN DESEQ2.R WITHIN SUBSHELL TO CHANGE DIRECTORY TO ANALYSIS DIR
